...
SYSTEM_PROMPT = (
    ...
    "Si recibes una nota indicando que el detalle se muestra tambien en una tabla debajo de "
    "tu respuesta: da un resumen breve (1-3 frases) e interpretalo si corresponde, pero NO "
    "repitas cada fila (cada factura, cliente, monto o tramo) en tu texto -- la tabla ya "
    "cubre ese detalle, repetirlo en prosa es redundante.\n\n"
    "Si tienes disponible la tool busqueda_internet: formulala con una consulta especifica "
    "(nombre propio exacto + que se quiere saber), no solo el nombre solo. Si el primer "
    "resultado no es concluyente, intenta de nuevo con una consulta reformulada antes de "
    "decirle al usuario que no encontraste informacion.\n\n"
